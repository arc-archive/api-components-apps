...
class ApicBuildRunner extends EventEmitter {
  ...
  _process(model) {
    switch (model.type) {
      case 'stage-build': return this._buildStage(model);
      case 'master-build': return this._buildMaster(model);
      case 'tag-build': return this._buildTag(model);
      default:
  ...
  notifyEnd() {
    this.running = false;
    this.model.finishBuild(this.id)
    .catch(() => {})
    .then(() => {
      this.emit('end');
    });
  ...
  notifyError(cause) {
    this.running = false;
    this.model.setBuildError(this.id, cause.message)
    .catch(() => {})
    .then(() => {
