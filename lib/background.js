...
function subscribe(cb) {
  ...
  getTopic()
  .then((topic) => pubsub.createSubscription(topic, subscriptionName))
  .then((sub) => {
    subscription = sub;
  ...
  getTopic((err, topic) => {
    ...
    publisher.publish(Buffer.from(JSON.stringify(data)), (err) => {
      if (err) {
        ...
        logging.info(`Test ${id} queued for background processing`);
      }
    });
  });
}

function dequeueTest(id) {
  getTopic((err, topic) => {
    if (err) {
      logging.error('Error occurred while getting pubsub topic', err);
      return;
    ...
    publisher.publish(Buffer.from(JSON.stringify(data)), (err) => {
      if (err) {
        ...
        logging.info(`Test ${id} dequeued from background processing`);
      }
    });
  });
}
