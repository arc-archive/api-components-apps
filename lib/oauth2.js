...
function extractProfile(profile) {
  // email is needed to check if the user has @mulesoft or @salesforce domain.
  const emails = profile.emails || [];
  let superUser = false;
  let orgUser = false;
  const suEmails = ['jarrodek@gmail.com', 'pawel.psztyc@gmail.com', 'ppsztyc@salesforce.com'];
  for (let i = 0; i < emails.length; i++) {
    ...
    if (info.value.indexOf('@mulesoft.com') !== -1 || info.value.indexOf('@mulesoft.com') !== -1) {
      orgUser = true;
      break;
    }
  }
