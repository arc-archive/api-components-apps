...
class ApicCiStatus extends PolymerElement {
      ...
      <app-header-layout has-scrolling-region id="scrollingRegion">
        <app-header fixed shadow scroll-target="scrollingRegion" slot="header">
          <app-toolbar>
            ...
            <template is="dom-if" if="[[!loggedIn]]">
              <a href="/auth/login?return=%2F" class="log-in-btn">
  ...
  _updateAuthReturn() {
    const anhor = this.shadowRoot.querySelector('.log-in-btn');
    if (!anhor) {
      return;
    }
    const rtn = encodeURIComponent(location.href);
